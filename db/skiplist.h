...
#ifndef LIGHTDB_SKIPLIST_H
#define LIGHTDB_SKIPLIST_H
...
typename SkipList<Key, Comparator>::Node *SkipList<Key, Comparator>::FindGreaterOrEqual(const Key &key,
  ...
  while (true) {
    ...
    if (KeyIsAfterNode(key, next)) {
      ...
      if (prev != nullptr) prev[level] = x;  // 在prev记录每一层比小于等于key的节点
      if (level == 0) {                      // 返回最底层大于/等于key的节点
        return next;
      } else {
        // Switch to next list
        level--;
...
typename SkipList<Key, Comparator>::Node *SkipList<Key, Comparator>::FindLessThan(const Key &key) const {
  ...
  int level = GetMaxHeight() - 1;
  while (true) {
    assert(x == head_ || compare_(x->key, key) < 0);  // 节点的key比参数key小
    Node *next = x->Next(level);
    if (next == nullptr || compare_(next->key, key) >= 0) {  // 节点key比参数key大
      if (level == 0) {
        return x;
      } else {
        // Switch to next list
...
void SkipList<Key, Comparator>::Insert(const Key &key) {
  ...
  if (height > GetMaxHeight()) {
    for (int i = GetMaxHeight(); i < height; i++) {  // 大于当年最高的高度，则对应的prev节点为head
      prev[i] = head_;
    }
    ...
    max_height_.store(height, std::memory_order_relaxed);  // 就算有并发读，也不会造成影响。
  }
